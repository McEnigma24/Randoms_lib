...
#define sizeof_imp(x) { cout << ": " << std::setprecision(1) << fixed << (x)/1000000000 << "gb " << (x)/1000000 << "mb " << (x)/1000 << "kb " << std::setprecision(0) << (x) << "b \n"; }
#define show_sizeof(class) { size_t size = sizeof(class); cout << #class; sizeof_imp((float)size); }
#define show_sizeof_many(class, count) { size_t size = sizeof(class) * count; cout << #class; sizeof_imp((float)size); }
